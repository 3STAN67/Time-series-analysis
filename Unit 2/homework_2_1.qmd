...
The output gap is the difference between the potential GDP and actual. Potential GDP is the level of output the economy can produce when operating at full capacity. This can be calculated with formulas that build on economic functions like the Cobb-Douglas. Real GDP measures the actual output of an economy adjusted for inflation. Nominal GDP is the total price and quantity of all sold goods and then it is adjusted for inflation to get the Real GDP. The difference between potential and real is the Output Gap. These are measured in US Dollar amounts.
:::
...
::: {.callout-note title="Answer" icon="false"}
Unemployment is measured with a Current Population Survey census and is conducted by Census Bureau on a monthly basis. It polls about 60,000 eligible homes across different geographical places in the US. Total unemployment can be broken down into 3 different sections: frictional, structural, and cyclical. Cyclical unemployment is unemployment created by downturns in the economy. Unemployment of all types are always reported as % of the labor force.
:::
