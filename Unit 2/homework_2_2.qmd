...
Data Collection Process: This data is collected in the monthly internet based M3 survey that collects data from manufacturing establishments with \$500 million or more in annual shipments. This covers 89 different industry categories and companies of varying sizes. The M3 has data on manufacturers value of shipment, new orders, end-of-month order backlog, end-of-month total inventory, materials and supplies, work-in-process, and finished goods inventories.

Unit of Analysis: This data is measured in millions of USD.

Meaning of Observations: This data can be interpreted as the dollar value of all materials and supplies held by manufacturers at a point in time.
:::
...
data_acf <- acf(manu_inv$manu_inv, plot = FALSE)
data_acov <- acf(manu_inv$manu_inv, type = "covariance", plot = FALSE)
...
::: {.callout-note title="Answer" icon="false"}
This is because autocovariance is in similar units where as autocorrelation is unit less and ranges between -1 and 1 making it easy to interpret. They both measure the relationship but on different scales.
:::
...
fhalf <- df %>% slice(1: midpoint)
shalf <- df %>% slice((midpoint + 1):n)

cat("First Half Statistics:\n")
cat("Mean:", mean(fhalf$manu_inv), "\n")
cat("Variance:", var(fhalf$manu_inv), "\n")
cat("Autocovariance:\n")
print(acf(fhalf$manu_inv, type = "covariance", plot = FALSE))

cat("\nSecond Half Statistics:\n")
cat("Mean:", mean(shalf$manu_inv), "\n")
cat("Variance:", var(shalf$manu_inv), "\n")
cat("Autocovariance:\n")
print(acf(shalf$manu_inv, type = "covariance", plot = FALSE))
...
::: {.callout-note title="Answer" icon="false"}
No because the mean, variance, and autocovariance are not constant throughout the data set.
:::
...
::: {.callout-note title="Answer" icon="false"}
Determining if a time series has a constant variance is so difficult because the actual data is only one realization of possibilities in the ensemble.
:::
