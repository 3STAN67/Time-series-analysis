...
x_t <- cumsum(rnorm(n, mean = 0, sd = 1))
...
ind_HW <- ind_ts %>% 
  model(Additive = ETS(ind_prod_indx ~ trend("A", alpha = 0.2, beta = 0.001) + error("A") + season("A", gamma = 0.01),
...
It is not very clear what auto regressive process the data is. I would guess a AR(1) based on no continuous significant lags.
:::
