...
pacman::p_load(nlme, tsibble, fable,feasts, tsibbledata,
               ...
               multilevelmod, broom.mixed, nanoparquet,
               nmonths
)
```
...
ggplot(
  data = food_bev_ts,
  aes(x = YearMon, y = sales_millions)
) +
  geom_line() +
  labs(
    title = "Food and Beverage Sales",
    y = "Sales (Millions)",
    x = "Date"
  ) +
  scale_y_continuous(
...
-   **State the bias correction procedure for log-transform estimates** When back-transforming log-scale estimates via exponentiation, bias arises because the exponential function is nonlinear. To correct this, obtain your log-scale point estimate μ̂ and residual variance estimate σ̂², then apply the bias correction factor e^(σ̂²/2) to yield the unbiased back-transformed estimate: e^(μ̂ + σ̂²/2). This correction is particularly important when residual variance is non-negligible. Confidence intervals can be constructed on the log scale and then back-transformed directly by exponentiating both limits, yielding appropriately asymmetric intervals on the original scale. -->
:::
...
```{r}
# Fit model (OLS)
sim_reduced_linear_lm1 <- food_bev_ts |>
  model(sim_reduced_linear1 = TSLM(log(sales_millions) ~ std_t + 
        sin1 + cos1 + sin2 + cos2 + sin3 + cos3))

# Compute forecast
n_years_forecast <- 5
n_months_forecast <- 12 * n_years_forecast
n_months <- nrow(food_bev_ts)

new_dat <- tibble(t = n_months:(n_months + n_months_forecast )) |>
  mutate(
    dates = seq(max(food_bev_ts$date), length.out=n_months_forecast + 1, by="1 month") 
  ) |>
  mutate(
    std_t = (t - mean(pull(food_bev_ts$sales_millions, t))) / sd(pull(food_bev_ts$sales_millions, t)),
