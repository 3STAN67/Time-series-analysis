...
```{r}
nominal_hw <- ngdp_ts |> 
  model(Additive = ETS(nominal_gdp ~ trend("A", alpha = 0.01, beta = 0.001) + error("A") + season("M", gamma = 0.05),
        opt_crit = "amse", nmse = 1))

report(nominal_hw)
```

I selected an additive model with multiplicative seasonality because, as the trend increases, the cyclical fluctuations in the data also grow in magnitude. A purely additive model would fail to capture this expanding seasonal variation, causing those effects to be absorbed into the error term instead of being modeled appropriately. 
:::
...
Examining the time series, I selected a smaller alpha value because business cycles include periods of expansion and recession. A higher alpha would cause the model to closely follow these short-term fluctuations, reducing the desired smoothing effect. The beta parameter controls trend smoothing, and I set it to 0.001 to prevent the model from overreacting to temporary deviations that typically revert to the mean. The data exhibits relatively mild seasonality that increases alongside the overall trend. A small gamma value allows the model to adjust gradually as the trend grows while filtering out short-term variations.
:::
...
I am confident in the model’s performance. I aimed for a specification that adapts to changes in the data without overreacting, and the results indicate that it effectively smooths the series while maintaining responsiveness to underlying trends.
...
real_hw <- rgdp_ts |> 
  model(Additive = ETS(real_gdp ~
        trend("A", alpha = 0.1, beta = 0.01) + error("A") + season("M", gamma = 0.001),
        opt_crit = "amse", nmse = 1))
...
I selected smaller parameter values for this model because, as indicated by the y-axis scale, this series evolves more gradually than nominal GDP. Since the underlying process changes at a slower pace, smaller smoothing parameters allow the model to adjust more cautiously, providing a smoother and more stable fit over time.
